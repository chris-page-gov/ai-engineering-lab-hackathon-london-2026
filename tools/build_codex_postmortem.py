    ...
    for ref in EXTERNAL_REFERENCES:
        ...
        if data:
            ...
            placeholder = textwrap.dedent(
                ...
                This local evidence placeholder was generated because the external source body could not be fetched during this build.

                - Canonical URL: {ref["canonical_url"]}
                - Fetch URL: {ref["fetch_url"]}
                - Fetch status: {status}
                - Fetch error: {error}

                The public postmortem uses citation-only source notes for this reference and does not depend on the copied source body.
                """
            )
            write_text(local_path, placeholder, read_only=True)
...
def write_public_walkthrough() -> None:
    text = textwrap.dedent(
        """\
        # Reader Walkthrough
    ...
    write_text(
        PUBLIC_WIKI_DIR / "walkthrough.md",
        frontmatter({"title": "Reader Walkthrough", "tags": ["walkthrough", "codex-postmortem-public"]}) + text,
    )
...
def write_public_index(
    ...
    parts = [
        ...
        f"- {rel_link(PUBLIC_WIKI_DIR / 'index.md', PUBLIC_WIKI_DIR / 'walkthrough.md', 'Reader Walkthrough')}\n",
