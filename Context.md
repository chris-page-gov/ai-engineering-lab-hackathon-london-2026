...
- Microsoft Copilot is included through a caveated Playwright web UI adapter because there is no stable headless local API for the installed desktop app. UI runs require an authenticated Playwright profile and may break when Microsoft changes selectors, loading states, tenant policies, or model routing.
