...
- `gov-ckan/`: generated full GOV.UK/data.gov.uk CKAN OKF bundle with `wiki/` OKF notes, compact chunked metadata under `data/`, screenshot examples under `wiki/assets/ui-examples/`, and a dedicated static `viewer.html`; published content is metadata and derived indexes, not raw harvest snapshots or downloaded resource bodies.
