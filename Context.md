...
The generated root `viewer.html` treats the URL hash as a global page route, not as a raw in-page anchor. Exchange aliases such as `#ex-0003` are unique routed pages and are validated by `scripts/update_viewer.py`; broader conversation IDs are intentionally not global routes because they appear on multiple reader, source, and exchange pages. Viewer graph relationships are derived from Markdown links and typed by link label plus target section, so they are useful navigation cues rather than a formal ontology. Rendered commit hashes and repo-relative code references link to GitHub where a stable repository target can be inferred.
