...
- The `gov-ckan/` bundle extends the OKF pattern to the National Data Library data.gov.uk CKAN directory as a metadata-first large-corpus bundle. It has its own static viewer because the existing root `viewer.html` is Markdown-page oriented; the CKAN viewer is built around searchable structural facets, filtered overview metrics, browser-style back/forward breadcrumbs, a non-hairball overview canvas, selected-item relationship graphs with cycling non-overlapping labels, spotlight-equivalent pointer/keyboard/touch interactions, data cards, and pinned comparison cards.
