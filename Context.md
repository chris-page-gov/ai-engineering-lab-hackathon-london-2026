...
- The `gov-ckan/` bundle extends the OKF pattern to the full National Data Library data.gov.uk CKAN directory as a metadata-first large-corpus bundle. It has its own static viewer because the existing root `viewer.html` is Markdown-page oriented and a single `okf-bundle.json` would be too large for this corpus. The CKAN publication path now uses `index.html` as the browser entry point, `okf-explorer.json` as the machine-readable large-corpus descriptor, and `data/overview.json` as the startup index. The default overview route must render from that small overview payload before the full dataset/resource indexes are hydrated; search, filters, deep links, and non-overview views load those indexes on demand, and Graph mode still lazy-loads relationship chunks. Overview recency previews validate CKAN date fields and fall back through `metadata_modified` and `metadata_created` so placeholder strings from harvested metadata cannot outrank real dates. The CKAN viewer is built around searchable SeeLinks-style folded structural facets, filtered overview metrics, visually separate history navigation and canvas mode controls, a non-hairball overview canvas, reduction views that work before an item is selected, lazy-loaded selected-item relationship graphs with colour keys, visible relationship lists, grouped edge labels, pan/zoom controls, clickable concept nodes, node-aware label placement, cycling conflict-only label layers, separate inspect-versus-recentre click behaviour, foldable side panels, spotlight-equivalent pointer/keyboard/touch interactions, data cards, and pinned comparison cards. URL state and CKAN-derived identifiers are treated as untrusted when rendered into the viewer; actions that carry those values use data attributes and event listeners rather than inline JavaScript. The static viewer treats GitHub Pages chunk delivery as a public-CDN dependency, so JSON chunk fetches retry transient `429`/`5xx` failures with cache-busting and conservative concurrency. `gov-ckan-sample/` preserves the pre-expansion 200-dataset sample as a smaller fallback bundle.
...
- `scripts/build_site.py`: builds the GitHub Pages-ready `_site/` bundle from root documentation, `viewer.html`, `challenge-2/wiki/`, Challenge 2 checked-in source-file trees, `postmortem-public/`, and the generated `gov-ckan/` and `gov-ckan-sample/` bundles while excluding ignored CKAN scratch folders such as `raw/`, `downloads/`, `resource-bodies/`, and transient OS metadata.
- `gov-ckan/`: generated full GOV.UK/data.gov.uk CKAN OKF bundle with `wiki/` OKF notes, compact chunked metadata under `data/`, screenshot examples under `wiki/assets/ui-examples/`, a browser entry point at `index.html`, a large-corpus descriptor at `okf-explorer.json`, and a dedicated static `viewer.html`; published content is metadata and derived indexes, not raw harvest snapshots or downloaded resource bodies. The viewer renders OKF Markdown notes in-app, supports light/dark mode, keeps single-click inspection separate from double-click graph navigation, exposes resource-row review, always renders static package-shaped JSON from the bundle, and can optionally fetch live `package_show` JSON from a data card without committing those raw responses.
- `gov-ckan-sample/`: preserved 200-dataset GOV.UK/data.gov.uk CKAN OKF sample bundle for fallback/demo comparison.
- `scripts/build_gov_ckan_bundle.py`: harvests paginated CKAN `package_search` metadata, normalises datasets/resources/publishers/facets/relationships, strips CKAN HTML metadata to readable text in the generated viewer, escapes CKAN-controlled metadata, writes the generated `gov-ckan/` bundle, and owns the generated viewer behaviour for in-app Markdown notes, theme mode, resource review, richer data cards, and timeline rendering.
...
- `postmortem-public/wiki/walkthrough.md`: illustrated first-reader route through the postmortem, Challenge 2 wiki, and local SeeLinks-style workbench, with explicit screenshot coverage gaps.
- `postmortem-public/wiki/readers/`: start-to-finish Markdown readers for each curated public Codex conversation.
...
The Codex postmortem source archive is evidence material, not automatically publication-ready content. Conversation transcripts may include local paths, screenshots, private workflow details, local assistant configuration references, and dynamic third-party source snapshots. The localized Karpathy X/gist copies have no explicit redistribution license recorded, so public releases should cite canonical URLs and publish metadata or short excerpts rather than full copied source bodies unless permission or an explicit license is obtained. The GitHub-safe `postmortem-public/` derivative redacts local paths, excludes raw transcripts, keeps `.claude/settings.local.json` visible as a conventional public path, and publishes citation-only external source notes. Repository artifact references in the postmortem use commit-specific GitHub fork permalinks where the source file was tracked at the tagged baseline; local-only sources are flagged as such. Postmortem conversation counts are curated Codex session-source counts from `tools/build_codex_postmortem.py`, not every local session in the Codex home directory. Finer-grained user/Codex turns are represented as sequenced exchanges, and `wiki/readers/` is now the standard GitHub-friendly route for reading each curated conversation start to finish without opening every exchange file. The public sanitizer is expected to redact local paths by role and path shape rather than a single workstation username, and generated source notes should consume the already-built exchange list so reader/exchange/source navigation remains one coherent build graph.

The generated root `viewer.html` treats the URL hash as a global page route, not as a raw in-page anchor. Exchange aliases such as `#ex-0003` are unique routed pages and are validated by `scripts/update_viewer.py`; broader conversation IDs are intentionally not global routes because they appear on multiple reader, source, and exchange pages. Viewer graph relationships are derived from Markdown links and typed by link label plus source/target section, so they are useful navigation cues rather than a formal ontology. Source-to-source links are typed as related-source relationships. Rendered commit hashes and repo-relative code references link to GitHub where a stable repository target can be inferred.

The public methodology sources now distinguish the original LLM-wiki pattern sources from later OKF standardization context. Karpathy's X/gist sources remain citation-only methodology predecessors; the GoogleCloudPlatform `knowledge-catalog/okf` subtree and Google Cloud's 2026-06-12 OKF blog post are recorded as citation-only context explaining how OKF formalizes the Markdown/frontmatter/wiki pattern as a portable knowledge-bundle standard. The postmortem builder owns those related-source links so future regeneration preserves the bidirectional Karpathy-to-OKF graph.

There is no separate Codex skill for this postmortem-reader workflow in the repository. The standard mechanism is the repository builder plus the generated `AGENTS.md` contracts in `postmortem/` and `postmortem-public/`.
