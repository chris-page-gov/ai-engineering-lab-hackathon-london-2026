...
- [SeeLinks Micropedia Parity](#seelinks-micropedia-parity)
...
## SeeLinks Micropedia Parity

Status: implemented in Dark Data Workbench as of 2026-05-12.

The side-by-side parity matrix is [notes/seelinks-micropedia-parity-matrix.md](notes/seelinks-micropedia-parity-matrix.md). It records how the original SeeLinks/Micropedia controls map into the Dark Data Workbench implementation: dynamic facets, order-by stack, metadata toggle, pin/reorder controls, value ordering, links, collections, printing, AI export, tile text, browser-only editing, grid/outline/graph/timeline/reading/table/checks views, docked detail, rollups, history reductions, and persistence.

External service surfaces such as MCP/API loading are represented as local/configuration states unless an endpoint is configured. Collections are stored in browser IndexedDB through Dexie; per-pack UI preferences are stored in `localStorage`; editing remains an in-browser working copy and does not write raw Challenge 2 sources, HMRC narrative files, or generated pack JSON.
