...
<section class="stage"><div class="mode"><button class="navBtn" id="backBtn" title="Back" aria-label="Back">Back</button><button class="navBtn" id="forwardBtn" title="Forward" aria-label="Forward">Forward</button><button data-mode="overview" class="active">Overview</button><button data-mode="force">Graph</button><button data-mode="timeline">Timeline</button><button data-mode="matrix">Publisher x Format</button><button data-mode="resources">Resource stack</button><span class="meta" id="count"></span><span class="crumbs" id="crumbs"></span></div><div class="canvas" id="canvas"></div><div class="pins"><button class="action" id="spreadPins">Spread pins</button> <button class="action" id="exportPins">Export pins</button><div class="pinStack" id="pins"></div></div></section>
...
let byDataset=new Map(),byResource=new Map(),byPublisher=new Map(),filters={},query="",mode="overview",selected=null,spotlight=null,pins=JSON.parse(localStorage.getItem("govCkanPins")||"[]"),spread=false,labelPhase=0,labelLayerCount=1;
...
function applyUrl(){const params=new URLSearchParams(location.search);query=params.get("q")||"";$("query").value=query;filters={};["publisher","format","license","tag","update_year","host","govuk_linked","resource_type","publisher_family"].forEach(k=>{if(params.get(k))filters[k]=new Set(params.getAll(k));});mode=["overview","force","timeline","matrix","resources"].includes(params.get("mode"))?params.get("mode"):"overview";if(params.getAll("pin").length){pins=params.getAll("pin");localStorage.setItem("govCkanPins",JSON.stringify(pins));}spread=params.get("spread")==="1";selected=null;const hash=decodeURIComponent(location.hash||"#overview");if(hash.startsWith("#resource/"))selected={kind:"resource",...byResource.get(hash.slice(10))};else if(hash.startsWith("#publisher/"))selected={kind:"publisher",...byPublisher.get(hash.slice(11))};else if(hash.startsWith("#dataset/"))selected={kind:"dataset",...byDataset.get(hash.slice(9))};}
...
function applyFacet(k,v,additive=false){const current=filters[k]||new Set();if(additive){current.has(v)?current.delete(v):current.add(v);filters[k]=current;}else if(current.size===1&&current.has(v)){filters[k]=new Set();}else{filters[k]=new Set([v]);}if(!filters[k].size)delete filters[k];pushUrl();render();}
function renderFacets(visible){$("facets").innerHTML=Object.entries(facets).map(([key,items])=>{const base=filteredDatasets(key),counts=new Map();base.forEach(d=>valuesFor(d,key).forEach(v=>counts.set(v,(counts.get(v)||0)+1)));return`<section class="facet"><h2><span>${esc(key.replaceAll("_"," "))}</span><span>${(filters[key]?.size)||0}</span></h2>${items.slice(0,24).map(item=>{const active=filters[key]?.has(item.value);const count=counts.get(item.value)||0;return`<button class="${active?"active":""}" data-facet="${attr(key)}" data-value="${attr(item.value)}" title="Click to switch this facet; Ctrl or Command click to add/remove">${esc(item.value)} <small>${count}</small></button>`;}).join("")}</section>`;}).join("");document.querySelectorAll("[data-facet]").forEach(b=>b.onclick=e=>applyFacet(b.dataset.facet,b.dataset.value,e.ctrlKey||e.metaKey));}
...
function renderNode(n,p,active,labelled){const label=labelled?`<text x="${p.x+20}" y="${p.y+4}">${esc(shortLabel(n.label,50))}</text>`:"";if(n.kind==="resource")return`<g class="node ${active?"active":""}" tabindex="0" data-open="${attr(n.open||"overview")}" data-spot-id="${attr(n.id)}"><title>${esc(n.label)}</title><rect x="${p.x-14}" y="${p.y-10}" width="28" height="20" rx="3" fill="${colourFor(n.kind)}"></rect><line class="cardLine" x1="${p.x-9}" y1="${p.y-3}" x2="${p.x+9}" y2="${p.y-3}"></line><line class="cardLine" x1="${p.x-9}" y1="${p.y+3}" x2="${p.x+6}" y2="${p.y+3}"></line>${label}</g>`;if(n.kind==="dataset")return`<g class="node ${active?"active":""}" tabindex="0" data-open="${attr(n.open||"overview")}" data-spot-id="${attr(n.id)}"><title>${esc(n.label)}</title><rect x="${p.x-17}" y="${p.y-12}" width="34" height="24" rx="5" fill="${colourFor(n.kind)}"></rect>${label}</g>`;return`<g class="node ${active?"active":""}" tabindex="0" data-open="${attr(n.open||"overview")}" data-spot-id="${attr(n.id)}"><title>${esc(n.label)}</title><circle cx="${p.x}" cy="${p.y}" r="${active?11:8}" fill="${colourFor(n.kind)}"></circle>${label}</g>`;}
function renderForce(visible){const model=graphNodesAndEdges(visible),w=720,h=560,pos=graphPositions(model,w,h),map=new Map(model.nodes.map(n=>[n.id,n])),labels=labelLayers(model.nodes,pos,model.centre);const edgeSvg=model.edges.map(e=>{const a=pos[e.source],b=pos[e.target],active=model.centre&&(e.source===model.centre||e.target===model.centre);return a&&b?`<line class="edge ${active?"active":""}" data-spot-id="${attr(e.source)}" x1="${a.x}" y1="${a.y}" x2="${b.x}" y2="${b.y}"><title>${esc(e.kind)}: ${esc((map.get(e.source)||{}).label||e.source)} -> ${esc((map.get(e.target)||{}).label||e.target)}</title></line>`:"";}).join("");const nodeSvg=model.nodes.map(n=>renderNode(n,pos[n.id]||{x:w/2,y:h/2},n.id===model.centre,labels.has(n.id))).join("");canvas.innerHTML=`<svg class="graph" viewBox="0 0 ${w} ${h}" role="img" aria-label="${model.centre?"Selected item relationship graph":"Reduced dataset publisher graph"}">${edgeSvg}${nodeSvg}</svg><p class="graphCaption">${model.centre?`Showing ${model.nodes.length} graph nodes directly related to ${esc(shortLabel((map.get(model.centre)||{}).label||model.centre,70))}.`:`Showing ${visible.slice(0,60).length} datasets plus their publishers. Select a dataset, resource, or publisher for a relationship graph.`} Labels rotate every two seconds when they would overlap.</p>`;bindRows(canvas);}
