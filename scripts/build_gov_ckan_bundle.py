...
The centre canvas never defaults to a hairball. `#overview` opens with a screen-sized bundle info-card. Dataset, publisher, resource-stack, timeline, matrix, and Graph views are available after selection or filtering.
...
let byDataset=new Map(),byResource=new Map(),byPublisher=new Map(),filters={},query="",mode="overview",selected=null,spotlight=null,pins=JSON.parse(localStorage.getItem("govCkanPins")||"[]"),spread=false,labelPhase=0,labelLayerCount=1,openFacet="";
...
function applyFacet(k,v,additive=false){const current=filters[k]||new Set();if(additive){current.has(v)?current.delete(v):current.add(v);filters[k]=current;}else if(current.size===1&&current.has(v)){filters[k]=new Set();}else{filters[k]=new Set([v]);}if(!filters[k].size)delete filters[k];openFacet=k;pushUrl();render();}
...
$("clearFilters").onclick=()=>{filters={};query="";openFacet="";$("query").value="";pushUrl();render();};
