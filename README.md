...
- [GOV.UK CKAN OKF bundle](gov-ckan/wiki/index.md): full metadata-first data.gov.uk CKAN bundle with a separate [large-corpus static viewer](gov-ckan/viewer.html#overview), folded SeeLinks-style facets, filtered overview metrics, separated history navigation, Graph/Timeline/Matrix/Stack reduction views, graph legend/relationship/pan/zoom controls, node-aware labels, inspect-versus-recentre clicks, foldable side panels, and readable metadata cards, designed to preserve derived concepts and provenance rather than copies of remote source documents. The preserved 200-dataset backup remains available at [GOV.UK CKAN sample bundle](gov-ckan-sample/wiki/index.md) and [sample viewer](gov-ckan-sample/viewer.html#overview).
...
- `scripts/build_gov_ckan_bundle.py` and `scripts/check_gov_ckan_bundle.py`: CKAN harvest/normalisation and generated-bundle validation. Use `python3 scripts/build_gov_ckan_bundle.py --sample 200 --out gov-ckan-sample` for a fast local sample build, or omit `--sample` for the full metadata harvest under `gov-ckan`.
