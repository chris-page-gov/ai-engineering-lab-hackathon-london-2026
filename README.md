...
- [GOV.UK CKAN OKF bundle](gov-ckan/wiki/index.md): metadata-first data.gov.uk CKAN bundle with a separate [large-corpus static viewer](gov-ckan/viewer.html#overview), folded SeeLinks-style facets, filtered overview metrics, separated history navigation, Graph/Timeline/Matrix/Stack reduction views, graph legend/relationship/pan/zoom controls, node-aware labels, inspect-versus-recentre clicks, foldable side panels, and readable metadata cards, designed to preserve derived concepts and provenance rather than copies of remote source documents.
