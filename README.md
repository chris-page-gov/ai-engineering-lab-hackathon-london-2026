...
- [GOV.UK CKAN OKF bundle](gov-ckan/wiki/index.md): full metadata-first data.gov.uk CKAN bundle with a generated [large-corpus OKF Explorer entry](gov-ckan/index.html), [static viewer](gov-ckan/viewer.html#overview), and [performance model](gov-ckan/wiki/performance.md). The default overview route loads a small overview payload first, then hydrates dataset/resource indexes only when search, filters, deep links, or non-overview views need them; Graph relationships remain lazy-loaded. Optional operational-metadata sidecars add provenance-bearing canonical publisher facts without rewriting the full search corpus. The companion [300-record stratified evaluation bundle](gov-ckan-sample/wiki/index.md) and [sample viewer](gov-ckan-sample/viewer.html#overview) cover publisher aliases, common-word decoys, benchmark queries, formats, missing metadata, and filter intersections with deterministic regeneration.
...
- `gov-ckan-sample/`: deterministic 300-dataset stratified evaluation bundle derived from the committed full corpus without network or model calls.
- `scripts/build_gov_ckan_bundle.py`, `scripts/build_gov_ckan_evaluation_sample.py`, `scripts/check_gov_ckan_bundle.py`, and `scripts/check_gov_ckan_performance.py`: CKAN harvest/normalisation, deterministic evaluation reduction, generated-bundle validation, and startup-budget validation. Run `python3 scripts/build_gov_ckan_evaluation_sample.py --check` to verify the committed evaluation bundle, or omit `--sample` from the full builder for a live full metadata harvest under `gov-ckan`.
