...
- [GOV.UK CKAN OKF bundle](gov-ckan/wiki/index.md): full metadata-first data.gov.uk CKAN bundle with a generated [large-corpus OKF Explorer entry](gov-ckan/index.html), [static viewer](gov-ckan/viewer.html#overview), and [performance model](gov-ckan/wiki/performance.md). The default overview route loads a small overview payload first, then hydrates dataset/resource indexes only when search, filters, deep links, or non-overview views need them; Graph relationships remain lazy-loaded. The bundle now documents its generated enrichment contract: stable logical concept IDs, canonical licence/format values, publisher authority records, controlled topics, explicit relationship kinds, quality signals, and provenance. The viewer includes in-app OKF Markdown notes, light/dark mode, resource-row review, richer CKAN data cards with extras, static package JSON fallback, opt-in live API JSON, and a chronological Timeline view. The preserved 200-dataset backup remains available at [GOV.UK CKAN sample bundle](gov-ckan-sample/wiki/index.md) and [sample viewer](gov-ckan-sample/viewer.html#overview).
...
- [HMRC Beyond the Hype presentation slides](research/hmrc-beyond-hype/import/beyond_hype_coding_assistants_public_sector_engineering.pptx): the selected slide deck for the 2026-05-12 public-sector engineering talk.
...
- OKF-compatible Markdown frontmatter and root `viewer.html` make the public postmortem and Challenge 2 wiki publishable as a static GitHub-friendly viewer with Narrative, Timeline, Graph, and Links views, a collapsible navigation rail, self-contained Mermaid flowchart rendering, rendered Markdown tables, browser-style history navigation, typed graph relationship inspection, hash-routed exchange links, GitHub permalinks for commit hashes and repo-relative code references, and cycling non-overlapping graph-label layers
...
- reading source metadata and source notes as rendered Markdown preview or raw text
...
- `gov-ckan/`: full metadata-first GOV.UK/data.gov.uk CKAN OKF bundle, chunked JSON data, OKF Markdown notes, screenshot-backed UI design notes, a generated `index.html` entry point, `okf-explorer.json` large-corpus descriptor, `data/overview.json` startup index, generated `data/analysis/overview.json` overview context for the generic OKF Explorer, and a separate no-backend viewer for large-corpus exploration with folded structural facets, selectable reduction views, direct resource review, chronological timeline, in-app Markdown notes, light/dark mode, graph concept filters, grouped relationship labels, bounded dense-graph labels, inspect-first graph browsing, foldable side panels, and graph pan/zoom inspection. Its generated enrichment contract covers stable dataset/resource/publisher concept paths, normalised licence and format fields, publisher authority records, controlled topics, explicit relationship kinds, quality metrics, and provenance fields. The historical CKAN/dark-data implementation remains here as the large fixture; the reusable OKF Explorer product and bundle contract live in [`ai-infrastructure-wiki`](https://github.com/chris-page-gov/ai-infrastructure-wiki).
- `gov-ckan-sample/`: preserved 200-dataset sample bundle kept as a smaller fallback/demo backup.
- `scripts/build_gov_ckan_bundle.py`, `scripts/check_gov_ckan_bundle.py`, and `scripts/check_gov_ckan_performance.py`: CKAN harvest/normalisation, generated-bundle validation, and large-corpus startup-budget validation. Use `python3 scripts/build_gov_ckan_bundle.py --sample 200 --out gov-ckan-sample` for a fast local sample build, or omit `--sample` for the full metadata harvest under `gov-ckan`.
...
- `research/hmrc-beyond-hype/import/beyond_hype_coding_assistants_public_sector_engineering.pptx`: explicitly selected presentation slide deck for the 2026-05-12 HMRC Beyond the Hype demo release.
