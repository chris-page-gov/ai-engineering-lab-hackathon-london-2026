...
- [GOV.UK CKAN OKF bundle](gov-ckan/wiki/index.md): metadata-first data.gov.uk CKAN bundle with a separate [large-corpus static viewer](gov-ckan/viewer.html#overview), folded SeeLinks-style facets, filtered overview metrics, separated history navigation, Graph/Timeline/Matrix/Stack reduction views, and readable metadata cards, designed to preserve derived concepts and provenance rather than copies of remote source documents.
...
- `gov-ckan/`: metadata-first GOV.UK/data.gov.uk CKAN OKF bundle, chunked JSON data, OKF Markdown notes, screenshot-backed UI design notes, and a separate no-backend viewer for large-corpus exploration with folded structural facets and selectable reduction views.
