...
````text
The `.obsidian/` folder is local workspace state, not part of the generated knowledge base, so I’m leaving it uncommitted. I also spotted a Finder `.DS_Store` in `challenge-2/tools/`; I’m removing that before staging the actual implementation files.
````
