...
````text
There are Finder `.DS_Store` files lying around again, including inside the vault. I’m removing only the workspace-visible ones before staging so the follow-up commit contains the requested docs/config, not macOS metadata.
````
